...
function OfferListBlock({offerList, activeCity}: TOfferListBlockProps) : JSX.Element {
  ...
  <b className="places__found">{offerList.length} places to stay in {activeCity}</b>
  <SortBlock/>
    ...
    <div className="cities">
      <div className={`cities__places-container${emptyClassName} container`}>
        ...
        <div className="cities__right-section">
          <Map activeOfferId={activeOfferId} offers={offerList} prefixName={'cities'} type={MAP_CENTER_TYPES[0]} cityName={activeCity}/>
        </div>
      </div>
    </div>
  );
